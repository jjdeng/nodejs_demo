  ...
  {{#each nav}}
    {{#if title}}
      {{#if isHeader}}
        ...
        {{/if}}
      {{else}}
        <li style="display:flex" class="{{#if hidden}}hide {{/if}}" data-group="{{group}}" data-name="{{name}}" data-version="{{version}}">
          <a href="#api-{{group}}-{{name}}" class="nav-list-item">{{title}}</a>
          <a style="width:20px" href="#api-{{group}}-{{name}}-2" class="nav-list-item">#</a>
        </li>
      {{/if}}
    {{/if}}
  {{/each}}
...
<script id="template-article-sample-request" type="text/x-handlebars-template">
    {{#if article.sampleRequest}}
      <h2 id="api-{{article.group}}-{{article.name}}-2">{{__ "Send a Sample Request"}}</h2>
